...
async function main() {
  ...
  const spreadsheet = await sheets.spreadsheets.create({
    requestBody: {
      properties: { title: 'ShipQuery — Shipment Tracker' },
      sheets: [{ properties: { title: 'Shipments' } }]
    }
  });
  ...
  // Step 2: Make it accessible (share with your Google account)
  // Replace with your actual Google email
  await drive.permissions.create({
    fileId: sheetId,
    requestBody: {
      role: 'writer',
      type: 'anyone'  // or use type: 'user', emailAddress: 'your@email.com'
    }
  });
  console.log('Sheet shared (anyone with link can edit)');

  // Step 3: Add headers and dummy data
  const headers = [
  ...
  await sheets.spreadsheets.values.update({
    ...
    requestBody: {
      values: [headers, ...dummyData]
    }
  });

  console.log(`Added ${dummyData.length} shipment records`);
